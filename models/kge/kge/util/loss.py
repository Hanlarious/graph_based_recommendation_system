...
class KgeLoss:
    ...
    def create(config: Config):
        ...
        elif config.get("train.loss") == "bce_focal":
            offset = config.get("train.loss_arg")
            if math.isnan(offset):
                offset = 0.0
                config.set("train.loss_arg", offset, log=True)
            return BCEWithLogitsKgeLoss(config, offset=offset, bce_type=None, use_focal=True)
...
class BCEWithLogitsKgeLoss(KgeLoss):
    def __init__(self, config, offset=0.0, bce_type=None, temperature=1.0, use_focal=False, focal_gamma=.9999, **kwargs):
    ...
    def __call__(self, scores, labels, **kwargs):
        ...
        if self._use_focal:
            bce_l = F.binary_cross_entropy_with_logits(scores.view(-1), labels_as_matrix.view(-1), reduction='none')
            pt = torch.clamp(torch.exp(-bce_l), min=1e-6, max=1-1e-6)
            losses = (torch.pow(1-pt, self.focal_gamma) * bce_l).sum()
