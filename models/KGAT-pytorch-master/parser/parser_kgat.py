...
def parse_kgat_args(hyper_args):
    ...
    parser.add_argument('--Ks', nargs='?', default='[1,3,5,10,100]',
                        help='Calculate metric@K when evaluating.')

    parser.add_argument('--use_focal', type=float, default=0.9999, 
                        help='0: Contrastive loss, 1: Contrastive loss with focal weight.')
    
    parser.add_argument('--test_file', type=str, default='valid.txt',
                    help='Specify the name of the file for test file (validation set for training).')

    
    args = parser.parse_args(hyper_args)

    save_dir = 'trained_model/KGAT/{}/embed-dim{}_relation-dim{}_{}_{}_{}_lr{}_pretrain{}_usefocal{}/'.format(
        args.data_name, args.embed_dim, args.relation_dim, args.laplacian_type, args.aggregation_type,
        '-'.join([str(i) for i in eval(args.conv_dim_list)]), args.lr, args.use_pretrain, args.use_focal)
    args.save_dir = save_dir
    args.pretrain_model_path = save_dir
    return args
