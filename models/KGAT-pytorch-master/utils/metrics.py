...
def calc_metrics_at_k(cf_scores, train_user_dict, test_user_dict, user_ids, item_ids, Ks):
    ...
    # compute MRR
    mrr_tmp = torch.tensor([], device=rank_indices.device)
    for i, u in enumerate(test_user_dict):
        mrr_tmp = torch.cat((mrr_tmp, torch.reciprocal(1+rank_indices[i, test_user_dict[u]])))
    metrics_dict['recip_rank'] = mrr_tmp.cpu().numpy()
