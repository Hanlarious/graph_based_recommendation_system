...
class KGAT(nn.Module):
    ...
    def calc_cf_loss(self, user_ids, item_pos_ids, item_neg_ids):
        ...
        if self.use_focal ==0:
            ...
            # gamma is hard coded right now
            fl_weight = torch.clamp(torch.sigmoid(pos_score - neg_score), min=1e-8, max=1-1e-8)
            cf_loss = (-1.0) * torch.pow(fl_weight, self.use_focal) * F.logsigmoid(pos_score - neg_score)
            cf_loss = torch.mean(cf_loss)
    ...
    def calc_kg_loss(self, h, r, pos_t, neg_t):
        ...
        if self.use_focal ==0:
            ...
            fl_weight = torch.clamp(torch.sigmoid(neg_score - pos_score), min=1e-8, max=1-1e-8)
            kg_loss = (-1.0) * torch.pow(fl_weight, self.use_focal) * F.logsigmoid(neg_score - pos_score)
            kg_loss = torch.mean(kg_loss)
